...
var _ = SIGDescribe("[Serial]Backend Storage", Serial, func() {
	...
	It("Should use RWO when RWX is not supported", func() {
		...
		By("Expecting the VMI to be migratable (RWO backend storage is now allowed for migration)")
		...
		// VMI with RWO backend storage should be migratable (migration may work on local storage)
