...
var _ = Describe("DRA Status Controller", func() {
	...
	Context("updateStatus", func() {
		...
		It("should return early if pod resource claim status is not filled", func() {
			pod.Spec.ResourceClaims = []k8sv1.PodResourceClaim{
				{
					Name:                      "test-rc",
					ResourceClaimTemplateName: pointer.P("test-rc"),
				},
			}
	...
	Context("pciAddressFromDeckhouseGPUDeviceName", func() {
		DescribeTable("derives the PCI address from a physical GPU device name",
			func(deviceName, expected string) {
				Expect(pciAddressFromDeckhouseGPUDeviceName(deviceName)).To(Equal(expected))
			},
			Entry("physical GPU", "gpu-00000000-4f-00-0", "00000000:4f:00.0"),
			Entry("another physical GPU", "gpu-00000000-d5-00-0", "00000000:d5:00.0"),
			Entry("partition device", "part-gpu-00000000-4f-00-0-p19-s0-n1", ""),
			Entry("time-slicing variant", "gpu-00000000-4f-00-0-ts", ""),
			Entry("mps variant", "gpu-00000000-4f-00-0-mps", ""),
			Entry("non-hex bus", "gpu-00000000-zz-00-0", ""),
			Entry("missing prefix", "00000000-4f-00-0", ""),
			Entry("empty", "", ""),
		)
	})
