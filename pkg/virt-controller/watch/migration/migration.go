...
func (c *Controller) updateStatus(migration, migrationCopy *virtv1.VirtualMachineInstanceMigration, vmi *virtv1.VirtualMachineInstance, pods []*k8sv1.Pod, syncError error) error {
	...
	if migration.IsFinal() {
		...
		if aborting {
			// virt-handler is unwinding a migration that already runs. The job stays in
			// flight until the source reports the outcome, as with a deleted migration:
			// failing it now leaves the target pod behind a final job and makes the next
			// migration of this VMI wait on a job nobody tracks anymore.
			msg := "Migration is being aborted because the volume migration was canceled."
			c.recorder.Eventf(migration, k8sv1.EventTypeWarning, controller.FailedMigrationReason, msg)
			log.Log.Object(migration).Warning(msg)
...
func (c *Controller) handleMigrationBackoff(key string, vmi *virtv1.VirtualMachineInstance, migration *virtv1.VirtualMachineInstanceMigration) error {
	...
	if backoff > 0 {
		...
		c.Queue.AddWithOpts(priorityqueue.AddOpts{Priority: pointer.P(activePriority), After: backoff}, key)
...
// cancellation marker. It reports whether the migration had already started, in
// which case only an abort was requested and virt-handler still has to unwind it.
//
...
func (c *Controller) handleTargetPodCreation(key string, migration *virtv1.VirtualMachineInstanceMigration, vmi *virtv1.VirtualMachineInstance, sourcePod *k8sv1.Pod) error {
	...
	if c.podExpectations.AllPendingCreations() > 0 {
		c.Queue.AddWithOpts(priorityqueue.AddOpts{Priority: pointer.P(activePriority), After: 1 * time.Second}, key)
		return nil
...
func (c *Controller) createAttachmentPod(migration *virtv1.VirtualMachineInstanceMigration, vmi *virtv1.VirtualMachineInstance, virtLauncherPod *k8sv1.Pod) error {
	...
	resourceClaims := controller.GetHotplugResourceClaims(vmi)
...
func (c *Controller) handlePendingPodTimeout(migration *virtv1.VirtualMachineInstanceMigration, vmi *virtv1.VirtualMachineInstance, pod *k8sv1.Pod) error {
	...
	if isPodPendingUnschedulable(pod) {
		...
		if secondsSpentPending >= unschedulableTimeout {
			...
			c.Queue.AddWithOpts(priorityqueue.AddOpts{Priority: pointer.P(activePriority), After: delay}, migrationKey)
		}
	}
...
func (c *Controller) enqueueMigration(obj interface{}) {
	...
	if migration.Status.Phase == virtv1.MigrationRunning {
		c.Queue.AddWithOpts(priorityqueue.AddOpts{Priority: pointer.P(activePriority)}, key)
	} else {
		c.Queue.Add(key)
	}
}
...
func (c *Controller) addPod(obj interface{}) {
	...
	c.Queue.AddWithOpts(priorityqueue.AddOpts{Priority: pointer.P(activePriority)}, migrationKey)
}
