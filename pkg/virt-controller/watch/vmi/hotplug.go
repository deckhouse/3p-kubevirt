...
func (c *Controller) handleHotplugVolumesAndResourceClaims(hotplugVolumes []*v1.Volume, hotplugResourceClaims []*v1.ResourceClaim, hotplugAttachmentPods []*k8sv1.Pod, vmi *v1.VirtualMachineInstance, virtLauncherPod *k8sv1.Pod, dataVolumes []*cdiv1.DataVolume) common.SyncError {
	...
	currentPod, oldPods := getActiveAndOldAttachmentPodsForVolumes(readyHotplugVolumes, hotplugAttachmentPods)

	if currentPod == nil && !hasPendingPods(oldPods) && (len(readyHotplugVolumes) > 0 || len(readyResourceClaims) > 0) {
		// TODO: fix rate limiting
		if rateLimited, waitTime := c.requeueAfter(oldPods, time.Duration((len(readyHotplugVolumes)+len(readyResourceClaims))/-10)); rateLimited {
...
func (c *Controller) createAttachmentPod(vmi *v1.VirtualMachineInstance, virtLauncherPod *k8sv1.Pod, volumes []*v1.Volume, resourceClaims []*v1.ResourceClaim) (*k8sv1.Pod, common.SyncError) {
	attachmentPodTemplate, _ := c.createAttachmentPodTemplate(vmi, virtLauncherPod, volumes, resourceClaims)
