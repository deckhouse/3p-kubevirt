...
func (c *Controller) sync(vmi *virtv1.VirtualMachineInstance, pod *k8sv1.Pod, dataVolumes []*cdiv1.DataVolume) (common.SyncError, *k8sv1.Pod) {
	...
	if !isTempPod(pod) && controller.IsPodReady(pod) {
		...
		if pod.DeletionTimestamp == nil && needsHandleVolumeOrResourceClaimHotplug(hotplugVolumes, hotplugResourceClaims, hotplugAttachmentPods) {
			...
			hotplugSyncErr = c.handleHotplugs(hotplugVolumes, hotplugResourceClaims, hotplugAttachmentPods, vmi, pod, dataVolumes)
...
func checkForContainerImageError(pod *k8sv1.Pod) common.SyncError {
	...
	for _, containerStatus := range containerStatuses {
		...
		if reason == controller.ErrImagePullReason || reason == controller.ImagePullBackOffReason {
			return common.NewSyncError(errors.New(containerStatus.State.Waiting.Message), reason)
		}
	}
	return nil
}
