...
func (app *SubresourceAPIApp) addResourceClaimRequestHandler(request *restful.Request, response *restful.Response, ephemeral bool) {
	...
	if !app.clusterConfig.HotplugHostDevicesWithDRAEnabled() {
...
func (app *SubresourceAPIApp) removeResourceClaimRequestHandler(request *restful.Request, response *restful.Response, ephemeral bool) {
	...
	if !app.hotplugVolumesEnabled() {
		writeError(k8serrors.NewBadRequest(hotplugVolumeNotEnabledError), response)
		return
	}
...
func (app *SubresourceAPIApp) vmiResourceClaimPatch(name, namespace string, resourceClaimRequest *v1.VirtualMachineResourceClaimRequest) *k8serrors.StatusError {
	...
	if ownedByVirtualMachine(vmi) && !app.ephemeralHotplugSupported() {
		return k8serrors.NewBadRequest(fmt.Sprintf("VMI %s/%s is owned by a VM", vmi.Namespace, vmi.Name))
	}
	...
	patchBytes, err := generateResourceClaimRequestPatchVMI(&vmi.Spec, resourceClaimRequest)
	if err != nil {
		return k8serrors.NewConflict(v1.Resource("virtualmachineinstance"), name, err)
	}
...
func verifyResourceClaimOptions(resourceClaims []v1.ResourceClaim, resourceClaimRequest *v1.VirtualMachineResourceClaimRequest) error {
	...
	if add != nil {
		...
		reqOrTemplateName = resourceClaimOrTemplateName(add.ResourceClaim)
	} else if remove != nil {
	...
	for _, resourceClaim := range resourceClaims {
		...
		case add != nil:
			...
			if resourceClaimNameOrTemplateExists(&resourceClaim, reqOrTemplateName) {
				return fmt.Errorf("Unable to add ResourceClaim [%s] because it already exists", reqOrTemplateName)
		...
		if remove != nil {
			if resourceClaimNameExists(resourceClaim, name) {
				...
				foundRemoveClaim = true
				break
			}
		}
	}

	if remove != nil && !foundRemoveClaim {
		return fmt.Errorf("Unable to remove volume [%s] because it does not exist", name)
	}
...
func generateResourceClaimRequestPatchVMI(vmiSpec *v1.VirtualMachineInstanceSpec, resourceClaimRequest *v1.VirtualMachineResourceClaimRequest) ([]byte, error) {
	return generateResourceClaimRequestPatch("", vmiSpec, resourceClaimRequest)
}

func generateResourceClaimRequestPatch(prefix string, vmiSpec *v1.VirtualMachineInstanceSpec, resourceClaimRequest *v1.VirtualMachineResourceClaimRequest) ([]byte, error) {
	resourceClaimPath := prefix + "/spec/resourceClaims"
	hostDevicePath := prefix + "/spec/domain/devices/hostDevices"
...
func addAddResourceClaimRequests(vm *v1.VirtualMachine, resourceClaimRequest *v1.VirtualMachineResourceClaimRequest, vmCopy *v1.VirtualMachine) error {
	...
	for _, request := range vm.Status.ResourceClaimRequests {
		if err := validateAddResourceClaimRequest(request, name); err != nil {
			return err
		}
	}
...
func validateAddResourceClaimRequest(request v1.VirtualMachineResourceClaimRequest, name string) error {
